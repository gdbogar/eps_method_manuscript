...
EPS_test <- ggplot(GH1_quantified_EPS, aes(x = EPS_ug_glu_g_soil_extract, y = EPS_ug_glu_g_soil_dilution_corr)) +
  geom_point() +
  geom_smooth(method = 'lm') +
  labs(x = "Extract EPS (µg glucose/g soil)", y = "Precipitate EPS (µg glucose/g soil)") +
  xlim(0, 420) +
  theme_classic() +
...
EPS_test <- ggplot(GH1_quantified_EPS, aes(x = EPS_ug_glu_g_soil_extract, y = EPS_ug_glu_g_soil_dilution_corr)) +
  ...
  xlab("Extract EPS (µg glucose/g soil)") +
  ylab("Precipitate EPS (µg glucose/g soil)") +
  theme_classic()

EPS_test

#print:
pdf(file = "Figures_Tables/Imputed_Pigments.pdf",
    width = 5, # The width of the plot in inches
    height = 3) # The height of the plot in inches
print(EPS_test)
dev.off()
